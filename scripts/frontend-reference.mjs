...
function renderOverview(reference) {
  let body = pageHeader('Фронтенд-документация');
  body += 'Этот раздел описывает структуру Angular-приложения: компоненты пользовательского интерфейса, сервисы, модели данных и маршруты. Он помогает быстро найти нужный экран, API-клиент или TypeScript-контракт при разработке и review.\n\n';
  body += '## Сводка\n\n| Раздел | Количество |\n| --- | --- |\n';
  ...
  body += '## Справочники\n\n';
  body += '- [Компоненты](./autogenerated/components)\n';
  body += '- [Сервисы](./autogenerated/services)\n';
  body += '- [Интерфейсы](./autogenerated/interfaces)\n';
  body += '- [Маршруты](./autogenerated/routes)\n\n';
  body += 'Компоненты и маршруты связаны с исходными файлами, поэтому по справочнику удобно переходить от функциональной зоны к конкретной реализации.\n';
  return body;
}
