...
function renderComponents(reference) {
  let body = pageHeader('Frontend Components');
  body += `Всего компонентов: **${reference.components.length}**.\n\n`;
  body += '| Component | Selector | Template | Styles | Imports | File |\n| --- | --- | --- | --- | --- | --- |\n';
...
function renderServices(reference) {
  let body = pageHeader('Frontend Services');
  ...
  for (const service of reference.injectables) {
    ...
    body += `File: \`${escapeMarkdown(service.file)}\`\n\n`;
...
function renderInterfaces(reference) {
  let body = pageHeader('Frontend Interfaces');
  ...
  for (const item of reference.interfaces) {
    ...
    body += `File: \`${escapeMarkdown(item.file)}\`\n\n`;
    if (item.extends) {
...
function renderRoutes(reference) {
  let body = pageHeader('Frontend Routes');
  body += `Всего route entries: **${reference.routes.length}**.\n\n`;
  body += '| Path | Component | Guard |\n| --- | --- | --- |\n';
...
function renderOverview(reference) {
  let body = pageHeader('Frontend Autogenerated Docs');
  ...
  body += `| Components | ${reference.components.length} |\n`;
  body += `| Injectable services | ${reference.injectables.length} |\n`;
  body += `| Interfaces | ${reference.interfaces.length} |\n`;
  body += `| Type aliases | ${reference.typeAliases.length} |\n`;
  body += `| Routes | ${reference.routes.length} |\n\n`;
  body += '## Разделы\n\n';
  body += '- [Components](/docs/frontend/autogenerated/components)\n';
  body += '- [Services](/docs/frontend/autogenerated/services)\n';
  body += '- [Interfaces](/docs/frontend/autogenerated/interfaces)\n';
  body += '- [Routes](/docs/frontend/autogenerated/routes)\n\n';
...
export async function writeFrontendReference({frontendRoot, docsOut}) {
  ...
  const targetDir = path.join(docsOut, 'frontend', 'autogenerated');
  await fs.mkdir(targetDir, {recursive: true});

  await fs.writeFile(path.join(docsOut, 'frontend', 'autogenerated.md'), renderOverview(reference));
